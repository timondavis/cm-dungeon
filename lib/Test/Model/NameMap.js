...
describe('NameMap', function () {
    ...
    it('will return contents as an array if asked', function () {
        map = new NameMap_1.NameMap();
        map.add(s1.name, s1);
        map.add(s2.name, s2);
        map.add(s3.name, s3);
        ...
        chai_1.expect(mapArray.length).to.be.equal(2);
        chai_1.expect(mapArray[0]).to.be.equal(s2);
        chai_1.expect(mapArray[1]).to.be.equal(s3);
    });
});
