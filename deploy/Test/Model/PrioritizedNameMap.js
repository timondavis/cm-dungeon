...
describe('PrioritizedNameMap', function () {
    ...
    it('should allow forEach traversal of all keys contained within the collection', function () {
        resetTest();
        var foundNames = {
            'value1': false,
            'value2': false,
        ...
        chai_1.expect(foundNames.value4).to.be.true;
        chai_1.expect(foundNames.value5).to.be.true;
        chai_1.expect(function () { return foundNames.value6; }).to.throw;
    });
    it('should allow forEach traversal of all keys within an indicated priority tier', function () {
        resetTest();
        var foundNames = {
            'value1': false,
            'value2': false,
    ...
    it('should allow items, indicated by key, to be moved to a different priority tier', function () {
        resetTest();
        map.add('value1', 64, 15);
        chai_1.expect(map.getKeyPriority('value1')).to.be.equal(15);
        chai_1.expect(map.get('value1')).to.be.equal(64);
        chai_1.expect(map.getMapForPriority(15).has('value1')).to.be.true;
        map.updatePriority('value1', 6);
        chai_1.expect(map.getKeyPriority('value1')).to.be.equal(6);
        chai_1.expect(map.get('value1')).to.be.equal(64);
        chai_1.expect(function () { return map.getMapForPriority(15); }).to.throw;
        chai_1.expect(map.getMapForPriority(6).has('value1')).to.be.true;
    });
    ...
    it('should accurately report the length (item count) of the collection', function () {
        resetTest();
        map.add('value1', 1, 1);
        map.add('value2', 2, 2);
        map.add('value3', 3, 3);
        map.add('value4', 4, 4);
        map.add('value5', 5, 5);
        chai_1.expect(map.length).to.be.equal(5);
        map.remove('value2');
        map.updatePriority('value1', 2);
        map.set('value3', 7);
        map.add('value6', 6);
        map.remove('value4');
        chai_1.expect(map.length).to.be.equal(4);
    });
    it('should report on the priority of a given key', function () {
        resetTest();
        map.add('value1', 1, 1);
        map.add('value2', 2, 2);
        map.add('value3', 3, 3);
        map.add('value4', 4, 4);
        map.add('value5', 5, 5);
        chai_1.expect(map.getKeyPriority('value1')).to.be.equal(1);
        chai_1.expect(map.getKeyPriority('value2')).to.be.equal(2);
        chai_1.expect(map.getKeyPriority('value3')).to.be.equal(3);
        chai_1.expect(map.getKeyPriority('value4')).to.be.equal(4);
        chai_1.expect(map.getKeyPriority('value5')).to.be.equal(5);
    });
});
